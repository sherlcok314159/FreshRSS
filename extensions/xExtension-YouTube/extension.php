...
final class YouTubeExtension extends Minz_Extension
{
	...
	public function init(): void {
		...
		if (Minz_Request::controllerName() === 'extension') {
			...
			Minz_View::appendScript($this->getFileUrl('fetchIcons.js', 'js'));
		}
	...
	public function setIconForFeed(FreshRSS_Feed $feed, bool $setValues = false): FreshRSS_Feed {
		...
		try {
			$path = $feed->setCustomFavicon(null, $this->getName(), true, $v);
			if ($path === null) {
		...
		try {
			$feed->setCustomFavicon($contents, $this->getName(), true, $v, true);
	...
	*/
			...
			if ($doc->loadHTML('<?xml encoding="utf-8" ?>' . $entry->content())) {
				...
				if ($descriptions->length > 0 && $descriptions[0] instanceof DOMNode) {
					$content .= '<p class="enclosure-description">' .
						nl2br(htmlspecialchars($descriptions[0]->nodeValue ?? '', ENT_COMPAT, 'UTF-8'), true) . '</p>';
				}
	...
	public function handleConfigureAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			$extAction = Minz_Request::paramString('extAction') ?: null;
			if ($extAction !== null) {
				$feedDAO = FreshRSS_Factory::createFeedDao();
				$feed = $feedDAO->searchById(Minz_Request::paramInt('id'));
				...
				$this->setIconForFeed($feed, $extAction === 'update_icon');
