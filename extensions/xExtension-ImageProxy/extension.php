...
final class ImageProxyExtension extends Minz_Extension {
	...
	public static function getProxyImageUri(string $url): string {
		...
		# 直接返回所有非反爬设定的
		# openai 用自带的 cloudflare 来反代；原生的不行
		if (strpos($url, 'openai') !== false) {
			return 'https://cdn.yunpengtai.top/' . $url;
		}
		if (strpos($url, 'cleverhans') !== false) {
			return 'https://wsrv.nl/?url=' . $url;
		}
		if (strpos($url, 'sinaimg') !== false) {
			$trimmed_url = str_replace('https://', '', $url);
			$new_url = 'https://i0.wp.com/' . $trimmed_url;
			return $new_url;
		}
		if (strpos($url, 'image.coolapk.com') !== false) {
			return 'https://xizeyoupan-simple-imag-61.deno.dev/api?url=' . $url;
		}
		return $url;
		if ($scheme === 'http') {
