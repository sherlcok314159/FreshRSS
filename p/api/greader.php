...
final class GReaderAPI {
	...
	private static function tagList() {
		...
		$tags = array(
			...
			array('id' => 'user/-/state/com.google/reading-list'),
			array('id' => 'user/-/state/org.freshrss/main'),
			array('id' => 'user/-/state/org.freshrss/important'),
			//array('id' => 'user/-/state/org.freshrss/hidden'),
	...
	private static function subscriptionList() {
		...
		foreach ($categoryDAO->listCategories(true, true) ?: [] as $cat) {
			foreach ($cat->feeds() as $feed) {
				$iconUrl = $feed->favicon(true);
				$priority = 'main';
				switch ($feed->priority()) {
					case FreshRSS_Feed::PRIORITY_IMPORTANT:
						$priority = 'important';
						break;
					case FreshRSS_Feed::PRIORITY_MAIN_STREAM:
						$priority = 'main';
						break;
					case FreshRSS_Feed::PRIORITY_CATEGORY:
						$priority = 'category';
						break;
					case FreshRSS_Feed::PRIORITY_ARCHIVED:
						$priority = 'archived';
						break;
					default:
						if (defined('FreshRSS_Feed::PRIORITY_FEED') && $feed->priority() >= FreshRSS_Feed::PRIORITY_FEED) {
							$priority = 'feed';
						}
						break;
				}
	 ...
	 * @phpstan-return array{'A'|'a'|'c'|'f'|'i'|'s'|'t',int,int,FreshRSS_BooleanSearch}
	 */
	...
	private static function streamContentsItemsIds(string $streamId, int $start_time, int $stop_time, int $count,
		...
		$entryDAO = FreshRSS_Factory::createEntryDao();
		$ids = $entryDAO->listIdsWhere($type, $id, $state, $searches, '0', '0', $order === 'o' ? 'ASC' : 'DESC', $continuation, 0, $count, 0);
		if ($ids === null) {
			self::internalServerError();
		}
	...
	private static function markAllAsRead(string $streamId, string $olderThanId) {
		...
		if (strpos($streamId, 'feed/') === 0) {
			...
			$entryDAO->markReadEntries($olderThanId, true);
		} elseif ($streamId === 'user/-/state/org.freshrss/main') {
			$entryDAO->markReadEntries($olderThanId, false, FreshRSS_Feed::PRIORITY_MAIN_STREAM);
		} elseif ($streamId === 'user/-/state/org.freshrss/important') {
			$entryDAO->markReadEntries($olderThanId, false, FreshRSS_Feed::PRIORITY_IMPORTANT);
		} elseif ($streamId === 'user/-/state/com.google/read') {
			$entryDAO->markReadEntries($olderThanId, false, null, null, null, FreshRSS_Entry::STATE_READ);
		} elseif ($streamId === 'user/-/state/com.google/unread') {
			$entryDAO->markReadEntries($olderThanId, false, FreshRSS_Feed::PRIORITY_HIDDEN + 1, null, null, FreshRSS_Entry::STATE_NOT_READ);
		} else {
			self::badRequest();
		}
		exit('OK');
