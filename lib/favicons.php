...
function downloadHttp(string &$url, array $curlOptions = []): string {
	...
	if ($proxy !== '') {
		...
		$isPrivate = ($noproxy === '') && preg_match('/^(127\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|localhost)/i', $host);
		if (!$isPrivate) {
...
function is_default_favicon(string $path): bool {
	return @filesize($path) === @filesize(DEFAULT_FAVICON)
		&& @md5_file($path) === @md5_file(DEFAULT_FAVICON);
}
...
function download_favicon(string $url, string $dest): bool {
	...
	if ($favicon != '' && file_put_contents($dest, $favicon) > 0) {
		return true;
	}
	if (!file_exists($dest)) {
