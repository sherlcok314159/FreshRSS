...
class FreshRSS_subscription_Controller extends FreshRSS_ActionController {
	...
	public function feedAction(): void {
		...
		if (Minz_Request::isPost()) {
			...
			$oldIconUrl = $feed->attributeString('icon_url') ?? '';
			$newIconUrl = trim(Minz_Request::paramString('icon_url'));
			$feed->_attribute('icon_url', $newIconUrl !== '' ? $newIconUrl : null);
			if ($newIconUrl !== $oldIconUrl) {
				require_once(LIB_PATH . '/favicons.php');
				@unlink(FAVICONS_DIR . $feed->hash() . '.ico');
			}
