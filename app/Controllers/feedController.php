	...
	public static function actualizeFeeds(?int $feed_id = null, ?string $feed_url = null, ?int $maxFeeds = null, ?SimplePie $simplePiePush = null): array {
		...
		foreach ($feeds as $feed) {
			...
			if (count($newGuids) > 0) {
				...
				// Check for same_link_read option: mark as read if link matches an already-read entry
				if (!$feed->hasAttribute('read_when_same_link_read')) {
				$userMarkWhen = FreshRSS_Context::userConf()->mark_when;
				if (isset($userMarkWhen['same_link_read'])) {
					$readWhenSameLinkRead = (int)$userMarkWhen['same_link_read'];
     } else {
					// Fallback to default configuration if not set in user config
					$defaultConf = FreshRSS_UserConfiguration::default();
					$readWhenSameLinkRead = (int)($defaultConf->mark_when['same_link_read'] ?? 0);
				}
				} elseif ($feed->attributeBoolean('read_when_same_link_read') === false) {
					$readWhenSameLinkRead = 0;
				} else {
					$readWhenSameLinkRead = $feed->attributeInt('read_when_same_link_read') ?? 0;
				}
				if ($readWhenSameLinkRead > 0) {
					$linksAsRead = array_fill_keys($feedDAO->listReadLinks($readWhenSameLinkRead), true);
				} else {
					$linksAsRead = [];
				}
				...
				foreach ($entries as $entry) {
					...
					// Fire entry_before_insert BEFORE the hash comparison so that any
					// content-mutating extension (e.g. PanguFormat) can normalise the
					// incoming entry first. Otherwise the stored hash (post-mutation)
					// never matches the freshly-parsed raw hash, every refresh looks
					// "updated" and updateEntry wipes _entry.attributes (killing user
					// data such as highlights).
					...
					if (isset($existingHashForGuids[$entry->guid()])) {
						...
						if (strcasecmp($existingHash, $entry->hash()) !== 0) {
							...
							$entry->applyFilterActions([], $linksAsRead);
							if ($readWhenSameTitleInFeed > 0) {
						...
						$entry->applyFilterActions($titlesAsRead, $linksAsRead);
						if ($readWhenSameTitleInFeed > 0) {
							$titlesAsRead[$entry->title()] = true;
						}
