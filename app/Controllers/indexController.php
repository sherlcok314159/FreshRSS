...
class FreshRSS_index_Controller extends FreshRSS_ActionController {
	...
	public static function listEntriesByContext(?int $postsPerPage = null): Traversable {
			...
			if (in_array(FreshRSS_Context::$sort, ['date', 'link', 'title'], true)) {
				...
				if ($pagingEntry !== null) {
					switch (FreshRSS_Context::$sort) {
						case 'date':
							$continuation_value = $pagingEntry->date(true);
							break;
						case 'link':
							$continuation_value = $pagingEntry->link(true);
							break;
						case 'title':
							$continuation_value = $pagingEntry->title();
							break;
					}
				}
		...
		foreach ($entryDAO->listWhere(
					...
					$id_min, FreshRSS_Context::$id_max, FreshRSS_Context::$sort, FreshRSS_Context::$order,
					FreshRSS_Context::$continuation_id, $continuation_value,
					$postsPerPage ?? FreshRSS_Context::$number, FreshRSS_Context::$offset) as $entry) {
			yield $entry;
		}
	}
