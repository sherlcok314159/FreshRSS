...
HTML;
	 ...
	 * @param array<string,bool|int> $linksAsRead Links of already-read entries (across all feeds)
	 */
	public function applyFilterActions(array $titlesAsRead = [], array $linksAsRead = []): void {
		$feed = $this->feed;
		if ($feed === null) {
			return;
		}
		if (!$this->isRead()) {
			...
			if (!empty($linksAsRead[$this->link()])) {
				Minz_Log::debug('Mark link as read (same link already read): ' . $this->link());
				$this->_isRead(true);
				Minz_ExtensionManager::callHook('entry_auto_read', $this, 'same_link_read');
			}
		}
