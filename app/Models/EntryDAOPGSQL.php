...
END $$;';
	...
	protected static function sqlRegex(string $expression, string $regex, array &$values): string {
		// PostgreSQL uses ~ for case-sensitive and ~* for case-insensitive matching
		$matches = static::regexToSql($regex);
		if (isset($matches['pattern'])) {
			$matchType = $matches['matchType'] ?? '';
			$pattern = $matches['pattern'];
			// PostgreSQL needs (?m) prefix for multiline mode
			if (str_contains($matchType, 'm')) {
				$pattern = '(?m)' . $pattern;
			}
			// Use ~* for case-insensitive matching, ~ for case-sensitive matching
			$operator = str_contains($matchType, 'i') ? '~*' : '~';
			$values[] = $pattern;
