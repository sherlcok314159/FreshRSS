...
SQL;
	...
	/**
	 * List links of read entries across all feeds for duplicate detection.
	 * Used to mark new entries as read if they have the same link as an already-read entry.
	 * @return array<int,string>
	 */
	public function listReadLinks(int $limit = 0): array {
		$sql = 'SELECT DISTINCT link FROM `_entry` WHERE is_read=1 AND link != \'\' ORDER BY id DESC'
			. ($limit < 1 ? '' : ' LIMIT ' . intval($limit));
		$res = $this->fetchColumn($sql, 0, []) ?? [];
		/** @var array<int,string> $res */
		return $res;
