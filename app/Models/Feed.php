...
class FreshRSS_Feed extends Minz_Model {
	...
	public const PRIORITY_HIDDEN = -20;
	...
	public function faviconPrepare(): void {
		...
		$iconUrl = $this->attributeString('icon_url');
		$ico = FAVICONS_DIR . $this->hash() . '.ico';

		if ($iconUrl !== null && $iconUrl !== '') {
			$ico_mtime = @filemtime($ico) ?: 0;
			$needsDownload = !file_exists($ico) || (is_default_favicon($ico) && ($ico_mtime < time() - 86400));
			if ($needsDownload) {
				$data = downloadHttp($iconUrl);
				if ($data !== '' && isImgMime($data)) {
					file_put_contents($ico, $data);
				} elseif (!file_exists($ico)) {
					@copy(DEFAULT_FAVICON, $ico);
				} else {
					@touch($ico);
				}
			}
		...
		if (FreshRSS_Context::$isCli) {
			...
			$should_download = ($ico_mtime == false) || ($ico_mtime < $txt_mtime) || (is_default_favicon($ico) && ($ico_mtime < time() - 86400));
			if ($txt_mtime != false && $should_download) {
				$url = file_get_contents($txt);
				if ($url != false) {
					download_favicon($url, $ico);
				}
			}
		}
	}
	...
	public function favicon(bool $absolute = false): string {
		return Minz_Url::display('/f.php?' . $this->hash(), 'html', $absolute);
	}
