...
final class FreshRSS_Context {
	...
	/** @var array{'all':bool,'starred':bool,'important':bool,'feed':int|false,'category':int|false,'tag':int|false,'tags':bool,'all_type':string} */
	...
	public static function updateUsingRequest(bool $computeStatistics): void {
		...
		$sort = Minz_Request::paramString('sort', true) ?: (FreshRSS_Context::userConf()->sort ?? 'date');
		self::$sort = in_array($sort, ['id', 'date', 'link', 'title', 'rand'], true) ? $sort : 'date';
	 ...
	 * @phpstan-return ($asArray is true ? array{'a'|'A'|'Z'|'c'|'f'|'i'|'s'|'t'|'T',bool|int} : string)
	 * @return string|array{string,bool|int}
	 */
	public static function currentGet(bool $asArray = false) {
		if (self::$current_get['all']) {
			$type = self::$current_get['all_type'];
			return $asArray ? [$type, true] : $type;
	...
	public static function _get(string $get): void {
		...
		switch ($type) {
			...
			case 'A':
				self::$current_get['all'] = true;
				self::$current_get['all_type'] = 'A';
				self::$name = _t('conf.query.get_A');
				self::$description = FreshRSS_Context::systemConf()->meta_description;
				self::$get_unread = self::$total_unread;
				break;
			case 'Z':
				self::$current_get['all'] = true;
				self::$current_get['all_type'] = 'Z';
				self::$name = _t('conf.query.get_Z');
				self::$description = FreshRSS_Context::systemConf()->meta_description;
				self::$get_unread = self::$total_unread;
				break;
			case 'i':
