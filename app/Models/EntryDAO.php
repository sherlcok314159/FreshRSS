...
SQL;
	...
	public function markReadEntries(string $idMax = '0', bool $onlyFavorites = false, ?int $priorityMin = null, ?int $prioritMax = null,
		...
		[$searchValues, $search] = $this->sqlListEntriesWhere('', $state, $filters);

		$sql .= $search;
		$this->registerSqlFunctions($sql);
		$stm = $this->pdo->prepare($sql);
...
SQL;
	...
	public function markReadFeed(int $id_feed, string $idMax = '0', ?FreshRSS_BooleanSearch $filters = null, int $state = 0, bool $is_read = true) {
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Minz_Log::error('SQL error ' . __METHOD__ . json_encode($info) . ' with SQL: ' . $sql);
			$this->pdo->rollBack();
			return false;
		}
		$affected = $stm->rowCount();
	...
	public function markReadTag(int $id = 0, string $idMax = '0', ?FreshRSS_BooleanSearch $filters = null,
		...
		[$searchValues, $search] = $this->sqlListEntriesWhere('e.', $state, $filters);
...
SQL;
			...
			if ($filter->getSearch() !== null) {
				foreach ($filter->getSearch() as $search_value) {
					if (static::isCompressed()) {	// MySQL-only
						...
						$values[] = "%{$search_value}%";
					} elseif (static::class === 'FreshRSS_EntryDAOSQLite' && mb_strlen($search_value) >= 3) {
						$sub_search .= 'AND ' . $alias . 'rowid IN (SELECT rowid FROM entry_fts WHERE entry_fts MATCH ?) ';
						$values[] = '"' . str_replace('"', '""', $search_value) . '"';
					} else {
			...
			if ($filter->getNotSearch() !== null) {
				foreach ($filter->getNotSearch() as $search_value) {
					if (static::isCompressed()) {	// MySQL-only
						...
						$sub_search .= 'AND ' . $alias . 'rowid NOT IN (SELECT rowid FROM entry_fts WHERE entry_fts MATCH ?) ';
						$values[] = '"' . str_replace('"', '""', $search_value) . '"';
					} else {
	 ...
	 * @param string|int $continuation_value
	...
	protected function sqlListEntriesWhere(string $alias = '', int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
			...
			string $continuation_id = '0', $continuation_value = 0): array {
		$search = ' ';
		$values = [];
	...
	private function sqlListWhere(string $type = 'a', int $id = 0, int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
			string $id_min = '0', string $id_max = '0', string $sort = 'id', string $order = 'DESC',
			string $continuation_id = '0', $continuation_value = 0, int $limit = 1, int $offset = 0): array {
		if (!$state) {
		...
		[$searchValues, $search] = $this->sqlListEntriesWhere('e.', $state, $filters, $id_min, $id_max,
			$sort, $order, $continuation_id, $continuation_value);
	...
	private function listWhereRaw(string $type = 'a', int $id = 0, int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
			string $id_min = '0', string $id_max = '0', string $sort = 'id', string $order = 'DESC',
			string $continuation_id = '0', $continuation_value = 0, int $limit = 1, int $offset = 0) {
		$order = in_array($order, ['ASC', 'DESC'], true) ? $order : 'DESC';
		$sort = in_array($sort, ['id', 'date', 'link', 'title', 'rand'], true) ? $sort : 'id';

		[$values, $sql] = $this->sqlListWhere($type, $id, $state, $filters, $id_min, $id_max, $sort, $order,
			$continuation_id, $continuation_value, $limit, $offset);
...
SQL;
		...
		if ($stm !== false && $stm->execute($values)) {
			...
			if ($this->autoUpdateDb($info)) {
				return $this->listWhereRaw($type, $id, $state, $filters, $id_min, $id_max, $sort, $order,
					$continuation_id, $continuation_value, $limit, $offset);
			}
			Minz_Log::error('SQL error ' . __METHOD__ . json_encode($info));
			return false;
	...
	public function listWhere(string $type = 'a', int $id = 0, int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
			string $id_min = '0', string $id_max = '0', string $sort = 'id', string $order = 'DESC',
			string $continuation_id = '0', $continuation_value = 0, int $limit = 1, int $offset = 0): Traversable {
		$stm = $this->listWhereRaw($type, $id, $state, $filters, $id_min, $id_max, $sort, $order,
			$continuation_id, $continuation_value, $limit, $offset);
		if ($stm !== false) {
			while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {
				if (is_array($row)) {
...
SQL;
	...
	public function listIdsWhere(string $type = 'a', int $id = 0, int $state = FreshRSS_Entry::STATE_ALL, ?FreshRSS_BooleanSearch $filters = null,
			...
			string $continuation_id = '0', $continuation_value = 0, int $limit = 1, int $offset = 0): ?array {
		[$values, $sql] = $this->sqlListWhere($type, $id, $state, $filters, $id_min, $id_max, 'id', $order,
			$continuation_id, $continuation_value, $limit, $offset);
		$stm = $this->pdo->prepare($sql);
