...
SQL;
	...
	protected static function sqlRegex(string $expression, string $regex, array &$values): string {
		// SQLite uses the REGEXP function which we register via registerSqlFunctions()
		// Pass the full pattern including delimiters, since PHP's preg_match needs them
		$values[] = $regex;
		return $expression . ' REGEXP ?';
	}

	/**
	 * Register the REGEXP function for SQLite using PHP's preg_match
	 */
	#[\Override]
	protected function registerSqlFunctions(string $sql): void {
		static $registered = false;
		if (!$registered && $this->pdo !== null && str_contains($sql, 'REGEXP')) {
			$registered = true;
			$this->pdo->sqliteCreateFunction('regexp', static function (?string $pattern, ?string $subject): bool {
